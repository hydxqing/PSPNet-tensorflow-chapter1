...
from model import resnet_v1_50
import math
import numpy as np
import cv2
...
parser.add_argument('--database', type=str, default='new_sonardata_resnet', help='SBD or Cityscapes.')
...
parser.add_argument('--lrn_rate', type=float, default=0.001, help='initial learning rate')
...
parser.add_argument('--train_max_iter', type=int, default=20000, help='Maximum training iteration')
parser.add_argument('--snapshot', type=int, default=5000, help='snapshot every ')
...
parser.add_argument('--gpu_num', type=int, default=10, help='gpu num')
...
parser.add_argument('--save_prediction', type=int, default=0, help='whether saving prediction.')
parser.add_argument('--ms', type=int, default=1, help='whether applying multi-scale testing.')
...
def train(resume_step=None):
    ...
    with tf.variable_scope(resnet):
        model = ResnetModel(num_classes,lrn_rate_ph, wd_rate_ph, wd_rate2_ph,mode='train', bn_epsilon=FLAGS.epsilon,
                            norm_only=FLAGS.norm_only,initializer=FLAGS.initializer,fix_blocks=FLAGS.fix_blocks,
                            fine_tune_filename=FLAGS.fine_tune_filename,wd_mode=FLAGS.weight_decay_mode,
...
def eval(i_ckpt):
    ...
    with tf.variable_scope(resnet):
        model = ResnetModel(num_classes, None, None, None,
                         mode='val', bn_epsilon=FLAGS.epsilon,
